...
#Checks to see if the security groups allow ingress from listed ports. 
def check_ports_open(from_port, to_port):
    if from_port <= RDPPORT and to_port >= RDPPORT:
        return True
    return False


#Checks the group permisssions to see if the group is compliant. 
...
#Checks groups to see if they allow RDP from anywhere. Prints non-compliant group names and deletes them.
